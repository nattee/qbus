...
(function (window, document, $) {
    ...
    var CurrentBalanceDonutChart = new Chartist.Pie(
        ...
        if (chart.type == 'slice') {
            var chartLength = chart.element._node.getTotalLength();

            chart.element.attr({
                'stroke-dasharray': chartLength + 'px ' + chartLength + 'px'
            });
            ...
            if (chart.index !== 0) {
                animation['stroke-dashoffset'].begin = 'anim' + (chart.index - 1) + '.end';
            }

            chart.element.attr({
                'stroke-dashoffset': -chartLength + 'px'
            });

            chart.element.animate(animation, false);
        }
    })
